...
openssl req -new -key client-key.pem -subj "/C=CA/ST=ONTARIO/L=TORONTO/O=ORG/OU=UNIT/CN=opensearch-client" -out client.csr
echo 'subjectAltName=DNS:opensearch-client' > client.ext
...
rm client.ext
# Permissions
chmod 600 node* root-ca.* client* admin*
